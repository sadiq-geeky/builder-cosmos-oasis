...
// Note: This creates a device_mappings table for IP->Name mapping
// You may want to create this table in your MySQL database:
/*
CREATE TABLE IF NOT EXISTS setcrmuis.device_mappings (
...
export const getDevices: RequestHandler = async (req, res) => {
  try {
    ...
    let query = `
      ...
      FROM setcrmuis.device_mappings
    `;
    let queryParams: any[] = [];
...
export const createDevice: RequestHandler = async (req, res) => {
  try {
    ...
    const query = `
      INSERT INTO setcrmuis.device_mappings (id, ip_address, device_name, created_on) 
      VALUES (?, ?, ?, NOW())
    `;
...
export const updateDevice: RequestHandler = async (req, res) => {
  try {
    ...
    const query = `
      UPDATE setcrmuis.device_mappings 
    ...
    const updatedDevice = await executeQuery<DeviceMapping>(
      "SELECT * FROM setcrmuis.device_mappings WHERE id = ?",
      [id],
    );
...
export const deleteDevice: RequestHandler = async (req, res) => {
  try {
    ...
    const query = "DELETE FROM setcrmuis.device_mappings WHERE id = ?";
